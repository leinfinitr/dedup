			...
			uint32_t hash_size, const char *root_path)
{
	...
	/*
	 * writing the main header. it will be overwriten in
...
int hashfile_chunking_method(struct hashfile_handle *handle)
{
...
int hashfile_hashing_method(struct hashfile_handle *handle)
{
...
static uint64_t skip_over_current_file_hashes(struct hashfile_handle *handle)
{
	...
	if (handle->header.version >= HASH_FILE_VERSION3 &&
		...
		chunk_info_block_size +=
			 sizeof(handle->current_chunk_info.size);
...
void hashfile_close(struct hashfile_handle *handle)
{
	...
	if (handle->omode == WRITE) {
		...
		/* finilizing the last file in the hashfile */
...
const struct chunk_info *hashfile_next_chunk(struct hashfile_handle *handle)
{
	...
	if (handle->header.version >= HASH_FILE_VERSION3 &&
		...
		ret = read(handle->fd, &handle->current_chunk_info.size,
			 sizeof(handle->current_chunk_info.size));
		if (ret != sizeof(handle->current_chunk_info.size)) {
			if (ret >= 0)
				errno = EAGAIN;
			return NULL;
		}
		...
		/*
		 * For fixed size chunking we (surprisingly) do not know the size
		 * of each chunk because of the tail chunks. So, just report 0
		 * irrespectful to hashfile version.
		 *
		 * Hashfile version 2 does not have chunk size even for
				 ...
				 const struct chunk_info *ci)
{
	...
	if (handle->header.chnk_method == VARIABLE) {
		ret = write(handle->fd, &ci->size, sizeof(ci->size));
		if (ret != sizeof(ci->size)) {
			if (ret >= 0)
				errno = EAGAIN;
			return -1;
		}
...
uint32_t hashfile_curfile_pathlen(struct hashfile_handle *handle)
{
	return handle->current_file.pathlen;
}
